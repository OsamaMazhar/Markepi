...
struct EXIFTokenParserTests {
    ...
    @Test("substitute({date}) resolves DateTimeOriginal → locale-aware date, no time")
    func resolvesDate() {
        ...
        // Formatted, not the raw EXIF string: EXIF writes "2026:06:18", a
        // formatter never does — so any colon at all is a giveaway now that
        // the clock reading lives in its own token.
        #expect(!result.contains(":"),
                "Date should be formatted and time-free. Got: '\(result)'")
    ...
    @Test("substitute({time}) resolves DateTimeOriginal → time to the minute")
    func resolvesTime() {
        let metadata = EXIFMetadataFactory.realisticMetadata(dateTime: "2026:06:18 14:30:00")
        let result = EXIFTokenParser.substitute("{time}", metadata: metadata)
        #expect(result.contains("30"), "Time should carry the minute. Got: '\(result)'")
        #expect(!result.contains("2026"), "Time should not carry the date. Got: '\(result)'")
        // Hours and minutes only, never seconds.
