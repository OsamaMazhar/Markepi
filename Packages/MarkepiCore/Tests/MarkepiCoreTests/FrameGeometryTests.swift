...
struct FrameGeometryTests {
    ...
    @Test("The framed canvas grows around the source, never into it")
    func framedCanvasGrows() {
        for style in FrameStyle.allCases {
            for size in [Self.portrait, Self.landscape] {
                ...
                // Every style adds a caption band, so height always grows.
                #expect(g.framedSize.height > size.height, "\(style) \(size) did not grow in height")
                // Width only where there is a side mat: `banner` has none, and
                // its photo bleeds to both edges by design.
                if style.hasSideMat {
                    #expect(g.framedSize.width > size.width, "\(style) \(size) did not grow in width")
                } else {
                    #expect(g.framedSize.width >= size.width,
                            "\(style) \(size) cropped the photo's width")
                    #expect(g.framedSize.width - size.width <= 1,
                            "\(style) should bleed to both edges, gained \(g.framedSize.width - size.width)px")
                }
            }
        }
    }
    ...
    @Test("The keyline is available in every style that offers one")
    func keylineAppliesToEveryStyleOfferingIt() {
        for style in FrameStyle.allCases where style.offersKeyline {
    ...
    @Test("Print draws no keyline, however the setting is left")
    func printNeverDrawsAKeyline() {
        // Its shadow already separates the photo from the mat; a heavy black
        // rule around a lifted print reads as a mistake. The user's stored
        // preference is untouched, so another style still honours it.
        var config = Self.config(style: .print, keyline: true)
        #expect(config.keylineEnabled, "the preference itself is kept, not cleared")
        #expect(FrameGeometry(config: config, sourceSize: Self.portrait).keylineWidth == 0)

        config.style = .classic
        #expect(FrameGeometry(config: config, sourceSize: Self.portrait).keylineWidth > 0,
                "switching away from print should find the keyline as the user left it")
    }
...
struct GalleryCaptionToneTests {
    ...
    func secondaryIsLighter() throws {
        ...
        let mat = WhiteFrameRenderer.matColor(for: WhiteFrameConfig(style: .gallery))
        let secondary = WhiteFrameRenderer.lighten(
            CGColor(gray: 0, alpha: 1), towards: mat, by: 0.45)
        let sRGB = CGColorSpace(name: CGColorSpace.sRGB)!
        func grey(_ color: CGColor) -> CGFloat? {
            color.converted(to: sRGB, intent: .defaultIntent, options: nil)?.components?[0]
        }
        let value = try #require(grey(secondary))
        // Stated as the relationship rather than a number: the mat's own tone
        // is a setting now, and what has to hold is that the secondary sits
        // between the primary and the ground it is read against.
        #expect(value > 0, "the secondary must be lighter than the black primary")
        #expect(value < (grey(mat) ?? 1), "and darker than the mat it sits on")
    }
}
