// markepi — command-line front end for the same MarkepiCore pipeline the app uses.
//
...
// Xcode never builds (it only builds the `MarkepiCore` library product).
    ...
    static func run(_ argv: [String]) async throws {
        ...
        if flags.has("--border") || flags.touched(prefix: "--border-") {
            config.whiteFrame = WhiteFrameConfig(
                ...
                // Left nil when the flag is absent, so both follow --border-mm
                // the way they do in the app. Restating the default here froze
                // them at the 8mm mat they were measured against.
                captionTextMillimetres: try flags.value("--border-caption-mm").map {
                    CGFloat(try doubleValue($0, flag: "--border-caption-mm", in: 0.5...20))
                },
                logoHeightMillimetres: try flags.value("--border-logo-mm").map {
                    CGFloat(try doubleValue($0, flag: "--border-logo-mm", in: 0.5...30))
                },
