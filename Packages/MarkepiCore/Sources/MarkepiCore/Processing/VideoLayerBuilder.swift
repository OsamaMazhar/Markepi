...
public struct VideoLayerBuilder {
    ...
    /// The transform a framed export's layer instruction needs, so the video
    /// lands in the hole the mat cuts for it.
    ///
    /// `AVVideoCompositionCoreAnimationTool` stretches the composition's
    /// finished frame to fill the video layer. Framing makes the composition's
    /// canvas larger than the video, so the frame arrives with the canvas's
    /// empty margin stretched in with it: the picture shrinks inside its own
    /// hole and the mat's window shows black beside it, which is what a framed
    /// video export looked like. Scaling to fill the canvas here and shrinking
    /// back there cancel out, and the video sits exactly in its hole.
    ///
    /// Deliberately a pure scale on top of the track's own transform, so it
    /// does not depend on which corner the composition's origin sits in:
    /// scaled to exactly the render size, the frame covers the canvas from
    /// whichever corner it is anchored to. An unframed export renders at the
    /// video's own size, where this is the identity and nothing changes.
    public static func fillTransform(
        preferred: CGAffineTransform,
        displayed: CGSize,
        renderSize: CGSize
    ) -> CGAffineTransform {
        guard displayed.width > 0, displayed.height > 0 else { return preferred }
        return preferred.concatenating(CGAffineTransform(
            scaleX: renderSize.width / displayed.width,
            y: renderSize.height / displayed.height))
    }
    ...
    public static func buildLayers(
        ...
        // Where the video sits inside the framed canvas, converted from the
        // hole's own rect: Core Animation's compositing space is y-up and
        // `photoRect` is y-down, so the vertical offset is measured from the
        // canvas top down to the hole's bottom edge.
        //
        // Derived from `photoRect` rather than from `bottom` for the reason the
        // photo path already learned: the canvas is rounded UP to even and that
        // spare row is added to the mat, so `bottom` puts the picture one row
        // low and leaves a line along its top edge — transparent in a photo,
        // and in a video the black behind the layers. Converting the hole's own
        // rect is aligned with it by construction.
        //
        ...
        videoLayer.frame = geometry.map {
            CGRect(x: $0.photoRect.minX,
                   y: $0.framedSize.height - $0.photoRect.maxY,
                   width: $0.photoRect.width,
                   height: $0.photoRect.height)
        } ?? parentLayer.bounds
