...
public struct VideoProcessor {
    ...
    public static func process(
        ...
        layerInstruction.setTransform(
            VideoLayerBuilder.fillTransform(preferred: compositionVideoTrack.preferredTransform,
                                            displayed: videoSize,
                                            renderSize: framedRenderSize),
            at: .zero)
    ...
    private static func extractCaptionMetadata(
        ...
        meta.merge(captionMetadata(model: model, make: make, creationDate: creationDate,
                                   locationISO6709: locationISO6709)) { a, _ in a }
        return meta
    }

    /// Shapes a video's device, date and location into the dictionary
    /// `EXIFTokenParser` reads for photos.
    ///
    /// Separated from the asset walk above so it can be tested without a file:
    /// what a video *carries* is AVFoundation's business, but what a caption
    /// makes of it is the frame's, and the two failed apart — the walk found
    /// the make and this step dropped it.
    static func captionMetadata(
        model: String?, make: String?, creationDate: Date?, locationISO6709: String?
    ) -> [String: Any] {
        var meta: [String: Any] = [:]

        // {camera_model} reads {TIFF}.Model; prefer the model, fall back to make.
        if let device = (model ?? make), !device.isEmpty {
            var tiff: [String: Any] = ["Model": device]
            // The make as well as the model: the brand mark and the maker's
            // name are both resolved from the make, so a video that carried
            // only the model got a caption with no logo and no maker — the one
            // thing a photo of the same scene always had.
            if let make, !make.isEmpty { tiff["Make"] = make }
            meta["{TIFF}"] = tiff
        }
