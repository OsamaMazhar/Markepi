...
public struct DeviceMetadataProvider {
    ...
    ///   - separator: What to put between the parts. Defaults to classic's
    ///     " · "; `print` passes plain spacing, which is what its reference
    ///     layout shows.
    ...
    public static func caption(
        ...
        // `.place` throughout: this builds a *frame caption*, which is drawn
        // into a context with a real foreground colour and so can show a
        // colour flag. The free text watermark keeps coordinates — see
        // `EXIFTokenParser.GPSFormat`.
        ...
        for field in CaptionField.allCases where fields.contains(field) {
            ...
            let value = field == .lens
                ? EXIFTokenParser.lensText(metadata: metadata,
                                           omitFocal: fields.contains(.focalLength),
                                           omitAperture: fields.contains(.aperture))
