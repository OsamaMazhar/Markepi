...
public struct ShareActionButton<ViewModel: WatermarkConfigurable & Observable>: View {
    ...
    public var body: some View {
        Group {
            ...
            case .batchProcessing(let current, let total, let fraction, let eta):
                batchProcessingView(current: current, total: total, fraction: fraction, eta: eta)
    ...
    /// Renders the `.batchProcessing(current:total:fraction:eta:)` state: progress bar,
    /// count, ETA, and a Stop Processing button.
    ///
    /// Extracted verbatim from `ControlsView.swift` lines 300–330.
    private func batchProcessingView(current: Int, total: Int,
                                     fraction: Double, eta: TimeInterval?) -> some View {
        VStack(spacing: 8) {
            HStack(spacing: 12) {
