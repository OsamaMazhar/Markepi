...
public enum BrandMarkRegistry {
    ...
    /// Brands whose key does not simply capitalise into their own styling.
    ///
    /// Everything else is `.capitalized`, which is right for the large
    /// majority ("canon" → "Canon", "fujifilm" → "Fujifilm"); only the ones
    /// that genuinely style themselves otherwise get an entry.
    ...
    /// The maker's name as they write it, for a brand key.
    public static func displayName(brandKey: String) -> String {
        displayNames[brandKey] ?? brandKey.capitalized
    }

    /// The maker's name for a photo, or nil when its metadata names no
    /// manufacturer the app recognises.
    ///
    /// Resolved through the same `brandKey` path that picks the brand mark, so
    /// a photo that earns a mark and a photo that earns a name always agree.
    public static func displayName(metadata: [String: Any]) -> String? {
        brandKey(metadata: metadata).map(displayName(brandKey:))
    }
