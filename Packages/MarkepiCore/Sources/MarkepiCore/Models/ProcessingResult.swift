...
/// and error presentation. Moved to MarkepiCore so both targets can import it.
public enum RenderingState: Equatable, Sendable {
    ...
    /// - Parameter fraction: How much of the batch's *work* is done, 0...1.
    ///   Carried separately from the count because they are different
    ///   questions: a bar driven by "3 of 4" stands still through a video that
    ///   is most of the batch.
    /// - Parameter eta: Estimated time remaining in seconds, or nil when insufficient data
    case batchProcessing(current: Int, total: Int, fraction: Double, eta: TimeInterval?)
    ...
    public static func == (lhs: RenderingState, rhs: RenderingState) -> Bool {
        ...
        case (.renderingVideo(let p1, let e1), .renderingVideo(let p2, let e2)):
            return p1 == p2 && e1 == e2
        case (.batchProcessing(let c1, let t1, let f1, let eta1),
              .batchProcessing(let c2, let t2, let f2, let eta2)):
            return c1 == c2 && t1 == t2 && f1 == f2 && eta1 == eta2
        case (.error, .error):
            return true
        default:
            return false
