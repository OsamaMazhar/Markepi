...
public struct WatermarkConfiguration: Sendable, Codable {
    ...
    /// What the user left behind on the frame styles they are not looking at,
    /// keyed by `FrameStyle.rawValue`.
    ///
    /// `whiteFrame` is always the style on screen; this holds the others, so
    /// setting a wide border on one style and a caption on another keeps both.
    /// A style nobody has visited has no entry and inherits the style on
    /// screen, which is what a strip of style previews has to show: the user's
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        // Only when there is something to say: a config that has never left its
        // first style writes no presets at all.
