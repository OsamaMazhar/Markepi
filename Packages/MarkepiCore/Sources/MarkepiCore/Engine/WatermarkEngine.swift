...
public actor WatermarkEngine {
    ...
    private func buildFilterGraph(
        ...
        if let frameConfig = config.whiteFrame, frameConfig.isEnabled {
            ...
            //
            // Derived from `photoRect`, not from `bottom`: the canvas is
            // rounded UP to even, and that spare row is added to the mat. Using
            // `bottom` put the photo one row low, leaving a transparent line
            // along its top edge that exported as a bright hairline — invisible
            // against a pale mat, obvious against a dark one. Converting the
            // hole's own rect is aligned with it by construction.
            let e = watermarkedResult.extent
            let photoOriginY = geometry.framedSize.height - geometry.photoRect.maxY
            let placedPhoto = watermarkedResult.transformed(
                by: CGAffineTransform(translationX: geometry.photoRect.minX - e.minX,
                                      y: photoOriginY - e.minY)
            )
            ...
            let matOffset = CGPoint(x: geometry.photoRect.minX - e.minX, y: photoOriginY - e.minY)
