...
- **[Risk] Bundled dataset grows the app** → Mitigation: a 1:110m simplified source is small by construction, in the low hundreds of KB, far smaller than a gazetteer would be. Verified against the built app size before shipping.

## Open Questions

- The exact public-domain source revision to build from, and how aggressively to simplify it — an implementation detail that doesn't change the spec, the approach, or the task breakdown; to be pinned in `tools/geo/README.md` when the script is built, and validated against the small-territory fixture list.
