...
- **A logo slot.** Vector-backed so it stays crisp at full export resolution. Ships the Apple mark in rainbow, black and white variants from official vector sources, plus `none`. The slot is pluggable — a neutral mark can replace the Apple one without touching the renderer.
...
- **Assets** — three vector logo variants added to the asset catalog. This is a classic pbxproj with manual file references, so new files and assets need explicit project registration.
- **Legal** — the Apple mark is a registered trademark; reproducing it in exported images carries App Store review risk. Recorded in design.md as an accepted risk, with `none` available and the slot kept pluggable.
